...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    Inputs.getShooterFarShoot().onTrue(new ShooterFarShoot());
    Inputs.getSourceIntake()
        .whileTrue(new IntakeFromShooter().andThen(new ShooterStop()).andThen(new IntakeSpinStop()));
    Inputs.getAutoIntakeDown().onTrue(new SequentialCommandGroup(new IntakeAutoPickup(),
        new ParallelRaceGroup(new IntakeFloorPos(), new IntakeWaitNote()), new IntakeAutoRetract()));
    // Inputs.getAutoIntakeDown2().onTrue(new SequentialCommandGroup(new
    // IntakeAutoPickup(), new IntakeWaitNote(), new IntakeAutoRetract()));
